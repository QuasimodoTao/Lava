...
void draw_char(int ch){
	...
	if(x + 8 >= width) {
		...
		y += 16;
		if(y >= high){
			memcpy(lfb,lfb + 16 * byte_per_scan_line,high * byte_per_scan_line - 16 * byte_per_scan_line);
			y -= 16;
		}
	}
	...
	for(dy = 0;dy < 16;dy++){
		for(dx = 0;dx < 8;dx++){
			...
			pos = (y + dy) * byte_per_scan_line + (x + dx) * byte_per_pixel;
			if(byte_per_pixel == 1)
				lfb[pos] = color;
...
void gui_init(u64 _mode_info){
	struct VESA_MODE * mode_info;
	mode_info = (struct VESA_MODE *)_mode_info;
	x = y = 0;
	high = mode_info->yres;
	width = mode_info->xres;
	byte_per_pixel = mode_info->bits_per_pixel/8;
	byte_per_scan_line = mode_info->byte_per_scan_line;
	lfb = (u8*)((u64)(mode_info->lfb));
	memset(lfb,0xff,high * byte_per_scan_line * byte_per_pixel);
}