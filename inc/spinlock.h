...
static inline void Lock##name (type * ptr){while(lock_bts(ptr,offsetof(type,member) * 8 + (off))) pause();}\
...
static inline void Lock##name (){while(lock_bts(ptr,off)) pause();}\
...
static inline void Lock##name (int bit_off){while(lock_bts(ptr,off + bit_off)) pause();}\
...
static inline void Lock##name (void * ptr){while(lock_bts(ptr,off)) pause();}\
...
static inline void Lock##name (type * array,int index){while(lock_bts(array,index*sizeof(type)*8 + off)) pause();}\
...
static inline void spin_lock_bit(void * ptr,u64 off){while(lock_bts(ptr,off)) pause();}
